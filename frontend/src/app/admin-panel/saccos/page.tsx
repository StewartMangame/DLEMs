...
export default function SaccosPage() {
  ...
  async function deactivate(id: number) {
    ...
    await fetch(`/api/admin-panel/saccos/${id}`, { method: "DELETE" });
    load();
  }

  async function activate(id: number) {
    await fetch(`/api/admin-panel/saccos/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "active" }) });
    load();
  }
...
function SaccoModal({ sacco, onClose, onSaved }: { sacco?: any; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const url = isEdit ? `/api/admin-panel/saccos/${sacco.id}` : "/api/admin-panel/saccos";
