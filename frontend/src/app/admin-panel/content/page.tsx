...
export default function ContentPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/admin-panel/content?${params}`)
      .then(r => r.json())
                  ...
                  <div className={styles.actionRow}>
                    ...
                    {s.status !== "needs_review" && (
                      <button className={`${styles.actionBtn} ${styles.warning}`} onClick={async () => {
                        await fetch(`/api/admin-panel/content/${s.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "needs_review" }) });
                        load();
                      }}>Flag Review</button>
                    )}
                  </div>
...
function EditStringModal({ str, onClose, onSaved }: { str: any; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    setSaving(true);
    await fetch(`/api/admin-panel/content/${str.id}`, {
...
function AddStringModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true);
    const res = await fetch("/api/admin-panel/content", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    setSaving(false);
